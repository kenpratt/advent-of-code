    ...
    <T as FromStr>::Err: Debug,
{
    ...
    pub fn coord_to_index(&self, coord: &Coord<T>) -> T {
        let x = coord.x - self.left;
        let y = coord.y - self.top;
        y * self.width + x
    }

    pub fn index_to_coord(&self, index: T) -> Coord<T> {
    ...
    <T as FromStr>::Err: Debug,
{
        ...
        F: FnMut(&char, &Coord<T>) -> Option<V>,
    {
        ...
        let mut grid = Self::new(bounds, Default::default);
    ...
    pub fn new<F>(bounds: Bounds<T>, initial: F) -> Self
    where
        F: FnMut() -> Option<V>,
    {
    ...
    pub fn coord_to_index(&self, coord: &Coord<T>) -> usize {
        cast!(self.bounds.coord_to_index(coord))
    }

    pub fn index_to_coord(&self, index: usize) -> Coord<T> {
        self.bounds.index_to_coord(cast!(index))
    }
